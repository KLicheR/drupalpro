...
zenity --info --text="Firefox is starting in order to create default and temp profiles.  Then Firefox will attempt to install the FEBE extention. Once FEBE is installed, choose the option to RESTART Firefox.  Next, choose Tools > FEBE > Restore Profile to restore the default profile using the file: ~/profileFx4{default}.fbu.\n  Finally, close Firefox "
